package MyFinalLib.FnList;

import MyFinalLib.FnList.*;
import MyFinalLib.FnA1sz.*;
import MyFinalLib.MyRefer.*;
...
//
	...
	<T extends Comparable<T>>
		...
		while (!nilq(xs) && !nilq(ys)) {
				int cmp = xs.hd().compareTo(ys.hd());
				if (cmp != 0) return cmp;
				xs = xs.tl();
				ys = ys.tl();
			}
			if (nilq(xs) && nilq(ys)) return 0;
			return nilq(xs) ? -1 : 1;
    }
//
    public static<T>
	FnList<T> fwork$make(Consumer<Consumer<? super T>> fwork) {
		MyRefer<FnList<T>> rf = new MyRefer<>();
		rf.set$raw(nil());
...
//
    public static<T>
	FnList<T> toArray(FnList<T> xs) {
		FnList<T> result = nil();
		while (!nilq(xs)) {
			result = cons(xs.hd(), result);
			xs = xs.tl();
		}
