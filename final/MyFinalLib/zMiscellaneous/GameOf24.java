...
//
	...
	// BFirstEnumerate implemented in Assign07_01
		...
		return FnGtreeSUtil.BFirstEnumerate(root).filter0(t -> t != null && Math.abs(t.eval() - 24.0) < 1e-6);
	}
	...
	// DFirstEnumerate implemented in Assign07_01
		...
		FnList<Term> all = makeTree(nums);
		FnGtree<Term> root = wrapAsTree(all);
		return FnGtreeSUtil.DFirstEnumerate(root).filter0(t -> t != null && Math.abs(t.eval() - 24.0) < 1e-6);
	}
//
