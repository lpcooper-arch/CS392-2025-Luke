...
public class MyQueueTwoList<T> extends MyQueueBase<T> {
    ...
    public void iforitm(BiConsumer<Integer, ? super T> action) {
      frnt.iforitm(action);
      final int[] index = {frnt.length()}; // changed to frnt.length() to fit with the FnList implementation in MyLibrary.FnList
    ...
    public void irforitm(BiConsumer<Integer, ? super T> action) {
      rear.iforitm(action);
      final int[] index = {rear.length()}; // changed to rear.length() to fit with the FnList implementation in MyLibrary.FnList
